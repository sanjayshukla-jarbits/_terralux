...
CORRECTED for compatibility with ModularOrchestrator and ExecutionContext.
...
# CORRECTED: Import hierarchy for _terralux project structure
try:
    ...
    try:
        ...
        try:
            # Try absolute import for when module is in orchestrator/steps/data_acquisition/
            import sys
            from pathlib import Path
            
            # Add _terralux root to path
            ...
            class BaseStep:
                ...
                def execute(self, context) -> Dict[str, Any]:
                    """Abstract execute method - CORRECTED signature"""
...
class CacheManager:
    ...
    def __init__(self, cache_dir: Optional[Path] = None):
        # CORRECTED: Use _terralux naming convention
        self.cache_dir = cache_dir or Path.home() / ".terralux_sentinel_hub"
...
class RealSentinelHubAcquisitionStep(BaseStep):
    ...
    def execute(self, context) -> Dict[str, Any]:
        ...
        CORRECTED to use ExecutionContext and return proper Dict format.
        """
        try:
            # Check if SentinelHub library is available
            if not SENTINELHUB_AVAILABLE:
                self.logger.warning("SentinelHub library not available, falling back to mock")
            ...
            # Create data request from context and hyperparameters
            request = self._create_data_request(context)
            ...
            if self.cache_manager.is_cached(cache_key) and not self.hyperparameters.get('force_download', False):
                ...
                self.set_output_data(context, 'imagery_data', str(data_path))
            ...
            # Download new data
            output_dir = Path(context.get_variable('output_dir', 'outputs'))
            ...
            # Store output in context
            self.set_output_data(context, 'imagery_data', str(cached_path))
    ...
    def _create_data_request(self, context) -> DataRequest:
        """
        Create data request from context and hyperparameters.
        CORRECTED to use ExecutionContext.
        """
        # Get parameters from context variables or hyperparameters
        bbox = context.get_variable('bbox') or self.hyperparameters.get('bbox')
        start_date = context.get_variable('start_date') or self.hyperparameters.get('start_date')
        end_date = context.get_variable('end_date') or self.hyperparameters.get('end_date')
    ...
    def _fallback_to_mock(self, context) -> Dict[str, Any]:
        """
        Fallback to mock implementation.
        CORRECTED to use ExecutionContext and proper format.
        """
        # Generate mock file path
        mock_path = f"/mock/path/{self.step_id}_sentinel.tif"
        
        # Store mock output in context
        self.set_output_data(context, 'imagery_data', mock_path)
        
        return {
            'status': 'success',
            'outputs': {
                'imagery_data': mock_path
            },
            'metadata': {
                'acquisition_date': context.get_variable('start_date') or self.hyperparameters.get('start_date', '2023-06-01'),
                'cloud_coverage': 15,
                ...
                'step_id': self.step_id,
                'step_type': self.step_type,
                'mock': True  # CORRECTED: For orchestrator detection
            }
        }


# Register the step - CORRECTED to ensure registration happens
try:
    ...
    logging.warning(f"Could not register RealSentinelHubAcquisitionStep: {e}")


# CORRECTED: Test execution with proper ExecutionContext mock
if __name__ == "__main__":
    ...
    print(f"Cache used: {result['metadata'].get('cache_used', False)}")
    print(f"Data path: {result['outputs']['imagery_data']}")
